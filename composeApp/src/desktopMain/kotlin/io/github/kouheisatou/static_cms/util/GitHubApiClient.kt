...
class GitHubApiClient {
    ...
    private val clientId: String by lazy { 
        getOAuthConfig("CLIENT_ID") ?: error("GitHub Client ID not configured. Please set GITHUB_CLIENT_ID environment variable or create oauth.properties file.")
    }
    private val clientSecret: String by lazy { 
        getOAuthConfig("CLIENT_SECRET") ?: error("GitHub Client Secret not configured. Please set GITHUB_CLIENT_SECRET environment variable or create oauth.properties file.")
    }
    ...
    private fun getOAuthConfig(key: String): String? {
        ...
        val possiblePaths = listOf(
            "oauth.properties",                    // 現在のディレクトリ
            "./oauth.properties",                  // 明示的な相対パス
            "../oauth.properties",                 // 親ディレクトリ
            "../../oauth.properties",              // 2つ上のディレクトリ
            System.getProperty("user.dir") + "/oauth.properties",  // ユーザーディレクトリ
            System.getProperty("user.home") + "/.staticcms/oauth.properties" // ホームディレクトリ
        )
        ...
        try {
            val resourceStream = this::class.java.classLoader.getResourceAsStream("oauth.properties")
    ...
    private suspend fun startCallbackServer() {
        callbackServer = embeddedServer(io.ktor.server.cio.CIO, port = 8080) {
            routing {
                get("/callback") {
                    ...
                    if (error != null) {
                        ...
                        authResultDeferred?.completeExceptionally(Exception("OAuth error: $error"))
                    } else if (code != null) {
                        call.respondHtml {
                            head {
                            ...
                            body {
                                div("window") {
                                    ...
                                    p("success") { +"✓ GitHub authentication completed successfully!" }
                                    p { +"This window will close automatically in 3 seconds..." }
                        ...
                        authResultDeferred?.completeExceptionally(Exception("Invalid callback"))
                    }
                }
            }
        }.start(wait = false)
    ...
    suspend fun createCommit(
        ...
        return try {
            val response = httpClient?.post("https://api.github.com/repos/$owner/$repo/git/commits") {
                headers {
                    append("Authorization", "Bearer $personalAccessToken")
                    append("Accept", "application/vnd.github.v3+json")
                    append("Content-Type", "application/json")
